 ...
 * the controller flow option. Stores the daemons UUID and daemons call state
 *
 ...
 * procedure call to that daemon again is ignored.
 *
 * @param[in] rpc_client to connect controller to daemon
 */
static void start_all_flows(xmlrpc_client *rpc_client)
{
	...
	/* Reports are fetched from the daemons based on the report interval duration */
	while (!sigint_caught) {
...
has_more_reports:
				...
				for (int i = 1; i < array_size; i++) {
					...
					if (rv) {
						...
						/* FIXME Kernel metrics (tcp_info).
						 * Other OS than Linux may not
						 * send valid values here. For
						 * the moment we don't care and
						 * handle this in the output/display
						 * routines. However, this
						 * do not work in heterogeneous
						 * environments */
