/*
 * trafgen.c - Routines used by the Flowgrind Daemon for advanced traffic generation
 *
...
inline static double calculate(struct _flow *flow,
	...
	case NORMAL:
		...
		DEBUG_MSG(LOG_DEBUG, "calculated normal distribution value %f "
			  "for parameters %f,%f", val, param_one, param_two);
		break;
	case UNIFORM:
		val = dist_uniform (flow, param_one, param_two );
		DEBUG_MSG(LOG_DEBUG, "calculated uniform distribution value %f",
			  val);
		break;
	case WEIBULL:
		val = dist_weibull (flow, param_one, param_two );
		DEBUG_MSG(LOG_DEBUG, "calculated weibull distribution value "
			  "%f for parameters %f,%f", val, param_one,
			  param_two);
		break;
	case EXPONENTIAL:
	...
	case PARETO:
		...
		DEBUG_MSG(LOG_DEBUG, "calculated pareto distribution value "
			  "%f for parameters %f,%f", val, param_one,
			  param_two);
		break;
	case LOGNORMAL:
		val = dist_normal (flow, param_one, param_two );
		DEBUG_MSG(LOG_DEBUG, "calculated lognormal distribution value "
			  "%f for parameters %f,%f", val, param_one,
			  param_two);
		break;
	case ONCE:
...
int next_request_block_size(struct _flow *flow) {
	...
	if (i >= MAX_RUNS_PER_DISTRIBUTION && bs < MIN_BLOCK_SIZE) {
		bs = MIN_BLOCK_SIZE;
		DEBUG_MSG(LOG_WARNING, "WARNING: applied minimal request size "
			  "limit %d for flow %d", bs, flow->id);
	}

	if (i >= MAX_RUNS_PER_DISTRIBUTION &&
	    bs > flow->settings.maximum_block_size) {
		bs = flow->settings.maximum_block_size;
		DEBUG_MSG(LOG_WARNING, "WARNING: applied maximal request size "
			  "limit %d for flow %d", bs, flow->id);
	}
...
int next_response_block_size(struct _flow *flow) {
	...
	if (bs && bs < MIN_BLOCK_SIZE) {
		bs = MIN_BLOCK_SIZE;
		DEBUG_MSG(LOG_WARNING, "applied minimal response size limit %d "
			  "for flow %d", bs, flow->id);
	}
	if (bs > flow->settings.maximum_block_size) {
		bs = flow->settings.maximum_block_size;
		DEBUG_MSG(LOG_WARNING, "applied maximal response size limit %d "
			  "for flow %d", bs, flow->id);
	}
